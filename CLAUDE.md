...
**`/guide` (`src/pages/GuidePage.tsx`) is the one route reachable without signing in**, it's the URL meant to be shared with a friend before they've ever opened the app. In `App.tsx` it's a sibling `<Route>` outside the `!profile ? <LoginPage> : <Routes>...` gate, not inside it, adding a new public route means adding another sibling there, not nesting inside the authenticated `<Routes>`. It uses real screenshots (`src/assets/guide/*.png`, captured from a demo account, not mocked up) for sign-in/add-card/scan, cropped via fixed-height `object-fit: cover` in `.guide-figure` rather than pre-cropped image files, so re-shooting a screenshot at a different content length doesn't require touching CSS. The "Add to Home Screen" section can't be screenshotted (that's native OS chrome, not page content), so it's hand-illustrated via `src/components/HomeScreenSteps.tsx` with generic (non-trademarked) share/menu icons instead. Guide prose lives in the normal `guide.*` i18n keys, same three-language rule as everywhere else, but re-shooting the screenshots for each locale wasn't worth it, they stay English regardless of selected language.
