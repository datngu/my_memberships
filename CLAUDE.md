...
**PWA config lives in `vite.config.ts`** (`VitePWA` plugin: manifest, service worker via Workbox `generateSW`, `registerType: 'autoUpdate'` so a new worker self-activates via skipWaiting/clientsClaim rather than waiting for the user). The manifest icon currently points at `public/favicon.svg` as a stand-in; before a real production deploy this should be swapped for proper 192/512px PNG (plus a maskable variant), see the icons array in `vite.config.ts`. A new worker activating doesn't repaint the page already open in the browser, that's what the "Check for update" button on `CardsPage` (`checkForAppUpdate` in `src/lib/pwa.ts`) is for: `registration.update()` then a hard reload. It's a no-op reload in dev (no service worker registered there, `npm run dev` doesn't enable `devOptions`), only meaningful against a built app.
